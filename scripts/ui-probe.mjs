...
const writeSummary = () => {
  ...
  const failed = Object.entries(assertions).filter(([, ok]) => !ok).map(([k]) => k);
  const maxSeen = report.maxSeen || {};
  const afterDebug = (report.after && report.after.debug) || {};
  const raf = report.rafStats || {};
  const summary = [
    '# CHUCO UI Probe Summary',
    '',
    `- Mode: ${report.mode}${report.scripted ? ' (scripted)' : ''}`,
    `- Assertions: ${Object.values(assertions).filter(Boolean).length}/${Object.keys(assertions).length} passed`,
    `- Failures: ${failed.length ? failed.join(', ') : 'none'}`,
    `- Max seen/caps: food ${maxSeen.maxFoodSeen ?? 'n/a'}/${afterDebug.maxFood ?? 'n/a'}, fish ${maxSeen.maxFishSeen ?? 'n/a'}/${afterDebug.maxFish ?? 'n/a'}, ripples ${maxSeen.maxRipplesSeen ?? 'n/a'}/${afterDebug.maxRipples ?? 'n/a'}`,
    `- RAF stats: p50=${(raf.p50 ?? 0).toFixed(2)}ms p95=${(raf.p95 ?? 0).toFixed(2)}ms max=${(raf.max ?? 0).toFixed(2)}ms samples=${raf.count ?? 0}`,
    `- Critical console errors: ${(report.criticalConsoleErrors || []).length}`,
    `- Page errors: ${(report.pageErrors || []).length}`,
    '',
    '- Screenshots: initial.png, after-interactions.png',
  ].join('\n');
  fs.writeFileSync(path.join(outDir, 'summary.md'), summary);
};

child.on('exit', (code) => {
  if ((code ?? 1) === 0) writeSummary();
  process.exit(code ?? 1);
});
