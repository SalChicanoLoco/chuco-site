...
Priority for the next agentic work block:

1. Web work first: keep improving Biblioteca/NM Socialists templates behind `make check`.
2. Chat app second: run `make chat-check`; if local Java/Clojure/API key are ready, run `make chat-live`, then `cd sena-chat && clj -M:run`.
3. Voice/Cloudflare deploys only when explicitly requested by Salvador and secrets are already configured.
4. Video renderer work should land behind its own check target before it becomes part of the default `make check`.
