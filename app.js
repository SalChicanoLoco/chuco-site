(() => {
  ...
  const food=[],ripples=[],fish=[],bubbles=[],plants=[],mouse={x:0,y:0}; let quality='auto', frameDt=16, lastTs=performance.now(); let lispProgram='(observe (edges trust play curiosity) (adapt reward residual))'; let dirtyEdges=false, lastPersistTs=0, softFailLogged=false, healthyFrames=0, lastSoftFailTs=0;
  const persist=()=>{
    try{
  ...
  window.__CHUCO_DEBUG__ = {
    snapshot: () => ({
      ...
      energy: chuco.energy,
      trust: chuco.trust,
      residual: chuco.rewardResidual,
      morphology: chuco.morphology,
      maxFood: MAX_FOOD,
      maxFish: MAX_FISH,
