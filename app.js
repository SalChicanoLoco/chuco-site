(() => {
  ...
  const ui = {mood:$('mood'),energy:$('energy'),trust:$('trust'),residual:$('residual'),graph:$('graph'),moodBar:$('moodBar'),energyBar:$('energyBar'),trustBar:$('trustBar'),morphologyHud:$('morphologyHud'),residualHud:$('residualHud'),trustHud:$('trustHud'),energyHud:$('energyHud'),researchPanel:$('researchPanel'),researchToggle:$('researchToggle'),quality:$('quality'),imgPrompt:$('imgPrompt'),loadSkin:$('loadSkin'),skinFile:$('skinFile'),aiSkin:$('aiSkin'),log:$('log'),lispTrace:$('lispTrace')};
  ...
  const food=[],ripples=[],fish=[],bubbles=[],plants=[],mouse={x:0,y:0}; const skinImage=new Image(); let hasSkinImage=false; let quality='auto', frameDt=16, lastTs=performance.now(); let lispProgram='(observe (edges trust play curiosity) (adapt reward residual))';
  const persist=()=>{
    try{
  ...
  async function generateAISkin(){
    const workerUrl=localStorage.getItem('chuco_worker_url')||prompt('Paste your Cloudflare Worker URL (https://...workers.dev)');
    if(!workerUrl)return;
    localStorage.setItem('chuco_worker_url',workerUrl);
    const skinPrompt=prompt('Image prompt for skin generation', 'Bioluminescent axolotl portrait, transparent background, high contrast, game-ready');
    if(!skinPrompt)return;
    log('Generating AI skin...');
    try{
      const res=await fetch(workerUrl.replace(/\/$/,'')+'/api/generate-axolotl-skin',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:skinPrompt})});
      const data=await res.json();
      if(!res.ok||!data.imageBase64) throw new Error(data.error||'Generation failed');
      skinImage.onload=()=>{hasSkinImage=true; log('AI skin generated and applied.');};
      skinImage.src='data:image/png;base64,'+data.imageBase64;
    }catch(err){log('AI skin generation failed: '+err.message);}
  }

  function copyImagePrompt(){
    const prompt=`Create a transparent-background axolotl sprite sheet for a web pet game. Style: bioluminescent fantasy aquarium, soft cyan-violet glow, readable face, high contrast, no background. Provide 4 rows: idle(16), swim(16), eat(12), happy(16). Each frame 320x320, consistent registration and scale.`;
    navigator.clipboard?.writeText(prompt).then(()=>log('Image-gen prompt copied. Paste into Deep Research / image model workflow.')).catch(()=>log(prompt));
  }
  function loadSkinFile(file){
    if(!file)return;
    const url=URL.createObjectURL(file);
    skinImage.onload=()=>{hasSkinImage=true; log('Generated skin loaded from file.'); URL.revokeObjectURL(url);};
    skinImage.onerror=()=>{hasSkinImage=false; log('Skin file could not be loaded.'); URL.revokeObjectURL(url);};
    skinImage.src=url;
  }
